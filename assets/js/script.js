    ...
    if ($("#contact-form").length) {
        $("#contact-form").validate({
            ...
            submitHandler: function (form) {
                $.ajax({
                    type: "POST", url: "mail.php", data: $(form).serialize(),
                    success: function () {
                        $( "#loader").hide(); $( "#success").slideDown( "slow" );
                        setTimeout(function() { $( "#success").slideUp( "slow" ); }, 3000);
                        form.reset();
                    },
                    error: function() {
                        $( "#loader").hide(); $( "#error").slideDown( "slow" );
            ...
            submitHandler: function (form) {
                ...
                if (typeof grecaptcha !== 'undefined') {
                    grecaptcha.ready(function() {
                        grecaptcha.execute('6Ldr0BctAAAAAA-6J2WxRZ1i3hEk1FQSx5Vfq9Gu', {action: 'submit'}).then(function(token) {
                            var formData = $(form).serialize() + '&recaptcha_token=' + token;
                            $.ajax({
                                type: "POST", url: "mail.php", data: formData,
                                success: function (response) {
                                    ...
                                    if(response.trim() === 'success') {
                                        ...
                                        $( "#error").html(response).slideDown("slow");
                                        setTimeout(function() { $( "#error").slideUp("slow"); }, 4000);
                                    }
                                },
                                error: function() {
                                    $( "#loader").hide();
                                    $( "#error").html("Network error occurred.").slideDown("slow");
                                    setTimeout(function() { $( "#error").slideUp("slow"); }, 3000);
                                }
                            });
                        });
                    ...
                    $( "#loader").hide();
                    $( "#error").html("reCAPTCHA failed to load.").slideDown("slow");
                    setTimeout(function() { $( "#error").slideUp("slow"); }, 3000);
                }
                return false;
            }
...
$(document).ready(function() {
    if (!localStorage.getItem('thoyiwa_cookie_consent')) {
        setTimeout(function() {
    ...
    $('#cookie-accept').on('click', function() {
        localStorage.setItem('thoyiwa_cookie_consent', 'accepted');
        $('#cookie-consent-banner').removeClass('show');
    });

    $('#cookie-decline').on('click', function() {
        localStorage.setItem('thoyiwa_cookie_consent', 'declined');
        $('#cookie-consent-banner').removeClass('show');
    });
});
